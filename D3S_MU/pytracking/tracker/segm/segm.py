    ...
    def track(self, image):
        ...
        if self.params.use_segmentation:
            if pred_segm_region is not None:
                return pred_segm_region, score_map.data.cpu().numpy()

        # Return new state
        new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        return new_state.tolist(), score_map.data.cpu().numpy()

    def track_updater(self, image):
        ...
        if self.params.use_segmentation:
            if pred_segm_region is not None:
                return pred_segm_region, score_map.data.cpu().numpy(), test_x, scale_ind, sample_pos, sample_scales, flag, s

        # Return new state
        new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        return new_state.tolist(), score_map.data.cpu().numpy(), test_x, scale_ind, sample_pos, sample_scales, flag, s
