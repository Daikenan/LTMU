...
from D3S_MU.D3s_MU import D3S_MU_Tracker
from D3S_MU.D3s import D3S_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        tic = time.time()
        if p.tracker == 'D3S_MU':
            tracker = D3S_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'D3S':
            tracker = D3S_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
